"""Groq API client — Qwen3-32B via OpenAI-compatible endpoint.
Free tier: 1K requests/day, 6K tokens/min.
"""
...
async def classify_batch(
    ...
    *,
    ...
    """Classify a batch using Groq Qwen3-32B (OpenAI-compatible)."""
    ...
    async with httpx.AsyncClient(timeout=120, trust_env=False) as client:
        ...
        logger.info("Groq %s: items=%d tokens=%d/%d latency=%dms",
            "judge" if use_judge else "primary", len(items), input_tokens, output_tokens, latency_ms)
        return LLMCallResult(items=items, model_used=GROQ_MODEL,
