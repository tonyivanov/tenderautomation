...
If `graphify-out/` exists, use it as the project knowledge graph.

Start with:
- `graphify-out/GRAPH_REPORT.md`
- `graphify-out/graph.json`

Prefer graph-based lookup for:
- dependency analysis
- call-flow analysis
- architecture questions
- impact analysis
- finding relationships between files, services, schemas and docs

Run `graphify update .` exactly from the root project folder after every codebase changing. Do not rebuild graph with subagents, use only `graphify update .`
